...
## 2026-09-27 手动操作日（纯 AI 手动 adb，未跑 Maa pipeline）

### 环境关键变更
- **MuMu 分辨率改为 1280×720**（用户手动改设置）：Physical 720×1280，截图 1280×720，文档所有坐标直接可用，无需换算。
- **截图铁律**：禁止 PowerShell `adb exec-out screencap -p > file.png`（二进制重定向损坏 PNG，导致 Read 读到错误画面、误判活动列表卡片位置并瞎滑十余次，用户当场纠正）。正确方式：`adb shell screencap -p /sdcard/x.png` → `adb pull /sdcard/x.png 本地路径`。
- adb 路径：`D:\Program Files\Netease\MuMu\nx_main\adb.exe`，设备 `127.0.0.1:16384`（另有 emulator-5554 勿误操作）。

### 酿月食香（部分完成，用户放弃第2局）
- 第1局完整走完：StageA 翻6盘 → StageB 配花（盲点黄桂花，3 PERFECT）→ StageC 3 SOSO → 结束游戏结算。体力 3→2。
- 第2局 StageB 卡住：漂浮花形月饼点击无反应（坐标正确但不弹花选单），误点托盘花形触发"现在一个月饼都没有获得哦！是否无收获结束此体力？"弹窗，取消按钮多次点不中。用户手动帮忙后进入 StageC（3 SOSO），结算完成。体力 2→1。
- **未决**：第2局 StageB 漂浮花形为何点不动（可能交互对象识别错误/动画时序）；托盘花形不是配花对象。今日剩余体力未继续。

### 巧手裁缝铺精选布料（2点体力全部用完）
- 两局流程一致：免费剪刀x12 → 第1关3堆（找到炫彩）→ 结算 → 遭遇事件 → 第2关4堆（找到炫彩）→ 结算 → 遭遇事件 → 第3关5堆（剪刀2→0退出）。
- 遭遇事件三型再验证：A型（压箱底好布料，开心宝）直接X；B型（布料堆里藏了好东西）点免费档变Get后X；C型（兑换商店六档全资源）整页X。
- 第2局第3关剪到鱼（"鱼缸已满，小鱼已放入仓库"弹窗）。
- 布料收获：黄37、蓝29、绿14（两局累计）。

### 巧手裁缝铺细密针脚（多单完成，带走1条鱼）
- **订单1（驯鹿人偶）满意度爆表**：157→183（第1局加急翻倍）→194（第2局无翻倍）→210/200（第3局加急翻倍），显示"满意度爆表！奖励x30"，点开心收下**带走顾客获得小鱼x1**。
- 订单4（柠檬茶包）：93→104（第1局），重接后10→20（第2局，重接消耗从蓝6+绿9变为黄x8）。
- 订单2（蓝色小兽）：59→71。
- 打法验证：第1关一键穿针x7、第2关一键穿针x8（线16≥N），第3关线1无法完成→X→确认结束（绝不暂时离开）。
- **关键坐标修正（1280×720实测）**：一键穿针按钮中心 **(840,605)**（不是文档的660,605）；完成本关 **(1122,640)**；开心收下 **(633,613)**；退出确认"确认" **(365,575)**；免费线档 **(281,573)**。
- 重接消耗浮动再验证：订单1 黄9→8、订单4 蓝6+绿9→黄x8（消耗项本身会变）。

### 百草灵仙赐福降世
- **完全未开始**。全新活动（NEW角标），需进入后读问号帮助、记录界面指纹与坐标、识别付费红线。

### 今日教训
- 每次操作前必须先截图识别当前画面状态，不能盲操作（用户多次纠正）。
- 弹窗按钮坐标要从截图精确测量，不能凭记忆。
- 一键穿针按钮位置文档记录有误，已修正为(840,605)。
